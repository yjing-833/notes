...
def get_type(schema: dict):
    # Handle cases where schema is not a dict (e.g. bool from additionalProperties)
    if not isinstance(schema, dict):
        # JSON Schema allows `true`/`false` as schemas
        # true  => any type allowed
        # false => no valid instance
        return "any" if schema is True else "never"
...
def _json_schema_to_python_type(schema: Any, defs) -> str:
    ...
    if schema is True:
        return "Any"
    if schema is False:
        return "Never"  # or raise APIInfoParseError if you prefer strict
